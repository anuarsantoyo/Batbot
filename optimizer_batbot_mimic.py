...
# Sensor calibration
calib = pd.read_csv('/home/anuarsantoyo/PycharmProjects/Batbot/experiments/biomimic/data/calib.csv', index_col=0).iloc[:,0]


# Start the CMA optimizer  # TODO: double check
pop_size = 10
n_generation = 50-13
save_directory = "experiments/biomimic/240409/test3/"  # TODO: dir
load = True
if load:
    file = open(save_directory+'optimizers/optimizer_12.pickle', 'rb')
...
for generation in range(generation_0, generation_0+n_generation):
    ...
    for i in range(optimizer.population_size):
        ...
        x = optimizer.ask()
        x_amp, y_amp, y_mid, delay = x  # TODO: dim

        cmd = (x_amp, y_amp, y_mid, delay)

        command_batbot_mimic(cmd, command_port)
        ...
        score = -fitness_avg_biomimic(measurements, plot=True, args={'peak_height': 0, 'peak_distance': 30})
        # minus as we want to maximize force
        ...
        df_dict_list.append({'Generation': generation,
                             ...
                             'X Amplitude': x_amp,
                             'Y Amplitude': y_amp,
                             'Y Neutral Angle': y_mid,
                             'Phase Shift': delay})  # TODO:dim
