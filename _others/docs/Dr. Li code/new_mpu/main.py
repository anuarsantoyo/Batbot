...
@micropython.native
def tick(tim):
    global wing_angle,imu_data,pitch,logfile,wing_angle_0,recv_channels,wing_span_mode,roll
    #t=time.ticks_ms()#test
    #获取输入
    #mpu_data=mpu.get_values()
    imu_data=imu.data_read_raw()
    imu_data.update(imu.data_read())
    ...
    #数据记录
    if LOG:
        logfile.write('%07d~wAng:%5.1f:imu:AcX:%f:AcY:%f:AcZ:%f:GyX:%f:GyY:%f:GyZ:%f:Rol:%f:Pth:%f:Yaw:%f:Hei:%f:~recv:Thr:%d:Pch:%d:Yaw:%d:Brk:%d:Est:%d:Llg:%d:Rlg:%d:Slg:%d\n'%\
                      (time.ticks_ms(),wing_angle,imu_data['AcX'],\
                       imu_data['AcY'],imu_data['AcZ'],imu_data['GyX'],\
                       imu_data['GyY'],imu_data['GyZ'],imu_data['Rol'],\
                       imu_data['Pth'],imu_data['Yaw'],imu_data['Alt'],recv_channels[THROTTLE_INPUT_CHANNEL],\
    ...
    if recv_channels[ESTOP_INPUT_CHANNEL]>CHANNEL_RANGE*0.7:
        ...
        servo_angle.stroke_estimate(wing_angle)
        ...
        #腿部控制
        L_leg_baseangle=45;L_leg_minangle=40;L_leg_maxangle=80      #minangle 和maxangle 表示左限位和右限位。调零位调这三个值
        R_leg_baseangle=55;R_leg_minangle=80;R_leg_maxangle=120
        if not recv_channels[BREAK_INPUT_CHANNEL]>CHANNEL_RANGE*0.7:#正常状态
            if USE_LEG_ROLL_PID:
                #try:
                #    roll_accel_angle=math.degrees(math.atan(imu_data['AcX']/imu_data['AcZ']))
                #    roll=roll_comp.filt(0.00003052*imu_data['GyY'],
                #                        roll_accel_angle,
                #                        0.5 if imu_data['AcZ']>200 else 0,
                #                        time=time.ticks_ms())
                #except:
                #    pass
                leg_Uangle_goal=roll_pid.calc(time.ticks_ms(),-imu_data['Rol'],
        ...
        #俯仰角控制
        #try:
        #    pitch_accel_angle=math.degrees(math.atan(imu_data['AcY']/imu_data['AcZ']))
        #    pitch=pitch_comp.filt(-0.00003052*imu_data['GyX'],
        #                          pitch_accel_angle,
        #                          0.5 if imu_data['AcZ']>200 else 0,
        #                          time=time.ticks_ms())
        #    #print(pitch)#test
        #except:
        #    pass
        tail_angle_goal=pitch_pid.calc(time.ticks_ms(),-imu_data['Pth'],
        ...
        #油门控制
        pca.duty(ESC_PWM_PORT,recv_channels[THROTTLE_INPUT_CHANNEL]/CHANNEL_RANGE*(120)+225) # 225变大，油门死区变窄
    else:
        for i in [0,1,2,3,4,5,6,8,9,10,11]:
            servos.position(i,degrees=90)
