...
#pid对象初始化
pitch_pid=pid.pid(0.7,0,0,pid_value=20,value_max=60,value_min=-60)    #改pid_value值可调整腿部舵机基准角度
...
def tick(tim):
    ...
    except:recv_channels[ESTOP_INPUT_CHANNEL]=0


    #数据记录
    if LOG:
        try:
            logfile.write('%07d~wAng%5.1f~mpu:AcX%6dAcY%6dAcZ%6dGyX%5dGyY%5dGyZ%5d~recv:Thr%4dRol%4dPch%4dYaw%4dEst%4d\n'%\
                          (time.ticks_ms(),wing_angle,mpu_data['AcX'],\
                           ...
                           recv_channels[ROLL_INPUT_CHANNEL],recv_channels[PITCH_INPUT_CHANNEL],\
                           recv_channels[YAW_INPUT_CHANNEL],recv_channels[ESTOP_INPUT_CHANNEL]))
        except:recv_channels[ESTOP_INPUT_CHANNEL]=0
    ...
    if recv_channels[ESTOP_INPUT_CHANNEL]>CHANNEL_RANGE*0.7:
        ...
        if wingSpan_angle<-50:
            wingSpan_angle=-50
        if wingSpan_angle>20:
            wingSpan_angle=20
        ...
        del(pitch_list[0])
        leg_baseAngle=pitch_pid.calc(sum(pitch_list)/len(pitch_list),45-recv_channels[PITCH_INPUT_CHANNEL]/CHANNEL_RANGE*90) #通过PID算出基准值
        leg_baseAngle+=0.5*wing_angle+20
        Lleg_angle=leg_baseAngle-(recv_channels[ROLL_INPUT_CHANNEL]-CHANNEL_RANGE/2)/CHANNEL_RANGE*30      #30表示滚转偏置角度范围
        Rleg_angle_N=leg_baseAngle+(recv_channels[ROLL_INPUT_CHANNEL]-CHANNEL_RANGE/2)/CHANNEL_RANGE*30     #30表示滚转偏置角度范围
        ...
        #油门控制
        thr=int(1189+1000*recv_channels[THROTTLE_INPUT_CHANNEL]/CHANNEL_RANGE)            #1190电调启动
        if (servo_angle.stroke==0 and wing_angle>5)\
           or (servo_angle.stroke==1 and wing_angle>30):
            thr-=int(700*recv_channels[THROTTLE_INPUT_CHANNEL]/CHANNEL_RANGE)     #100表示下扑过程减速量，在此相当于下扑速度是上扑速度的五分之四
